...
struct _ERAPI_SPRITE {
  ...
  u8 frames_per_bank; //    0x04 0x04 0x02 0x02 0x04
  ...
  u8 total_frames;    // 0x03 0x04 0x02 0x02 0x04
};
...
static const FUNC_ERAPI_MemAlloc ERAPI_MemAlloc =
    (FUNC_ERAPI_MemAlloc)(0x0080055B0 + 1);
static const FUNC_ERAPI_MemFree ERAPI_MemFree =
    (FUNC_ERAPI_MemFree)(0x08005598 + 1);

// JPN+ E-Reader Memory functions
// static const FUNC_ERAPI_MemAlloc ERAPI_MemAlloc =
// (FUNC_ERAPI_MemAlloc)(0x08005600 + 1); static const FUNC_ERAPI_MemFree
// ERAPI_MemFree = (FUNC_ERAPI_MemFree)(0x080055E8 + 1);
...
#define ERAPI_PlaySoundSystem(sound_id) ERAPI_FUNC_X2(0x105, sound_id)
//            a: Sound Index
...
#define ERAPI_SoundPause(sound_id) ERAPI_FUNC_X2(0x116, sound_id)
//            a: Sound Index
#define ERAPI_SoundResume(a) ERAPI_FUNC_X2(0x117, a)
//            a: Sound Index
#define ERAPI_PlaySoundSystemAtVolume(sound_id, volume)                        \
  ERAPI_FUNC_X3(0x118, sound_id, volume)
...
#define ERAPI_FadeIn(frame_count) ERAPI_FUNC_X2(0x200, frame_count)
#define ERAPI_FadeOut(frame_count) ERAPI_FUNC_X2(0x201, frame_count)
#define ERAPI_LoadBackgroundSystem(bg_layer, bg_id)                            \
  ERAPI_FUNC_X3(0x210, bg_id, bg_layer)
#define ERAPI_SetBackgroundOffset(bg_layer, x, y)                              \
  ERAPI_FUNC_X4(0x211, bg_layer, x, y);
#define ERAPI_SetBackgroundAutoScroll(bg_layer, x, y)                          \
  ERAPI_FUNC_X4(0x212, bg_layer, x, y);
...
#define ERAPI_SpriteCreateSystem(palette_index, sprite_id)                     \
  ERAPI_FUNC_X3(0x230, palette_index, sprite_id)
...
#define ERAPI_SpriteFree(sprite_handle) ERAPI_FUNC_X2(0x231, sprite_handle)
#define ERAPI_SetSpritePos(sprite_handle, x, y)                                \
  ERAPI_FUNC_X4(0x232, sprite_handle, x, y)
#define ERAPI_SpriteFrameNext(a) ERAPI_FUNC_X2(0x234, a)
//            a: Sprite Handle
#define ERAPI_SpriteFramePrev(a) ERAPI_FUNC_X2(0x235, a)
//            a: Sprite Handle
#define ERAPI_SetSpriteFrame(handle, frame) ERAPI_FUNC_X3(0x236, handle, frame)
#define ERAPI_SetSpriteAutoMove(a, b, c) ERAPI_FUNC_X4(0x239, a, b, c)
#define ERAPI_SpriteAutoAnimate(sprite_handle, speed_in_frames, flags)         \
  ERAPI_FUNC_X4(0x23C, sprite_handle, speed_in_frames, flags)
//            flags: 0 = Start Animating Forever
//               1 = Stop Animation
//               2 > Number of frames to animate for -2 (ex. 12 animates for 10
//               frames)
...
#define ERAPI_SpriteAutoRotateByTime(sprite_handle, angular_velocity, frames)  \
  ERAPI_FUNC_X4(0x240, sprite_handle, angular_velocity, frames)
// frames - 0: rotate forever
//        - 1: stop rotating
//        - n: rotate for n-2 frames
...
#define ERAPI_SpriteCreateCustom(palette_index, sprite)                        \
  ERAPI_FUNC_X3(0x24D, palette_index, (u32)sprite)
#define ERAPI_SpriteMove(a, b, c) ERAPI_FUNC_X4(0x257, a, b, c)
#define ERAPI_SpriteAutoScaleUntilSize(sprite_handle, scale, frames)           \
  ERAPI_FUNC_X4(0x25B, sprite_handle, scale, frames)
...
//            a: Sprite Handle
//            b: 0 = Hide , 1 = Show
//            NOTE - Not recommended, use ERAPI_SpriteShow/ERAPI_SpriteHide
//            instead

#define ERAPI_SetBackgroundPalette(palette_data, pal_index, offset, count)     \
  ERAPI_FUNC_X4(0x27E, (u32)palette_data, ((pal_index) << 8) | offset, count)
#define ERAPI_GetBackgroundPalette(a, b, c, d)                                 \
  ERAPI_FUNC_X4(0x27F, (u32)a, ((b) << 8) | c, d)
//            a: u16* Palette Data
//            b: u8 Palette Index (0-15)
//            c: u8 Color Index (0-15)
//            d: u8 Number of colors

#define ERAPI_SetSpritePalette(palette, palette_index, num_colors)             \
  ERAPI_FUNC_X4(0x280, (u32)palette, palette_index, num_colors)
#define ERAPI_PaletteSpriteSet(a, b, c, d)                                     \
  ERAPI_FUNC_X4(0x280, (u32)a, ((b) << 8) | c, d)
//            a: u16* Palette Data
//            b: u8 Palette Index (0-15)
//            c: u8 Color Index (0-15)
//            d: u8 Number of colors
...
#define ERAPI_PaletteSpriteGet(a, b, c, d)                                     \
  ERAPI_FUNC_X4(0x281, (u32)a, ((b) << 8) | c, d)
//            a: u16* Palette Data
//            b: u8 Palette Index (0-15)
//            c: u8 Color Index (0-15)
//            d: u8 Number of colors
...
#define ERAPI_CreateRegion(bg_index, pal_index, x, y, w, h)                    \
  ERAPI_FUNC_X4(0x290, ((bg_index) << 8) | pal_index, ((x) << 8) | y,          \
                ((w) << 8) | h)
//            a: 0,2=normal, 1,4,5,6,10=nothing, 3=messes with background tile
//            memory b: 0= no change, 4 = text grey if color is set, * text is
//            black c: x position in tiles d: y position in tiles e: width in
//            tiles f: height in tiles
#define ERHACK_RegionListClearUS() ((void (*)(void))(0x08037CC0 + 1))()
#define ERAPI_SetRegionColor(region, color_index)                              \
  ERAPI_FUNC_X3(0x291, region, color_index)
#define ERAPI_ClearRegion(region) ERAPI_FUNC_X2(0x292, region)
...
//            b: x position in tiles
//            c: y position in tiles
#define ERAPI_DrawLine(a, b, c, d, e, f)                                       \
  ERAPI_FUNC_X5(0x295, a, ((b) << 8) | c, ((d) << 8) | e, f)
//            a: Region
//            b: start x position in pixels
//            c: start y position in pixels
//            d: end x position in pixels
//            e: end y position in pixels
//            f: frame count to draw line over

#define ERAPI_DrawRect(region_handle, x1, y1, x2, y2, fill)                    \
  ERAPI_FUNC_X4(0x296, ((region_handle) << 8) | fill, ((x1) << 8) | y1,        \
                ((x2) << 8) | y2)
#define ERAPI_SetTextColor(region, foreground, background)                     \
  ERAPI_FUNC_X3(0x298, region, ((foreground) << 8) | background)
#define ERAPI_DrawText(region_handle, x, y, text)                              \
  ERAPI_FUNC_X4(0x299, region_handle, ((x) << 8) | y, (u32)text)
...
//            b: Text
//            Return: Pixel count
#define ERAPI_GetTextWidthEx(a, b, c)                                          \
  ERAPI_FUNC_X3(0x2C1, ((a) << 8) | c, (u32)b)
#define ERAPI_02C2(a) ERAPI_FUNC_X2(0x2C2, (u32)a)
#define ERAPI_02C3(a) ERAPI_FUNC_X2(0x2C3, a)
#define ERAPI_DecompressVPKOrNonVPK(vpk, dest)                                 \
  ERAPI_FUNC_X3(0x2DD, (u32)vpk, (u32)dest)
...
#define ERAPI_FlashLoadUserData(slot, data)                                    \
  ERAPI_FUNC_X3(0x11B, slot, (u32)data)
#define ERAPI_FlashSaveUserData(slot, data)                                    \
  ERAPI_FUNC_X3(0x11C, slot, (u32)data)
...
#define ERAPI_CalcDistanceBetweenPoints(a, b, c, d)                            \
  ERAPI_FUNC_X5(0x2B2, a, b, c, d)
...
#define ERAPI_ScanDotCode(input_buffer) ERAPI_FUNC_X2(0x2C2, (u32)input_buffer)
//            Return: u8 Status (0: ok, 1: read error, 4: timeout, 5: wrong
//            region)

#define ERAPI_SpriteSetPosAnimatedDuration(a, b, c, d)                         \
  ERAPI_FUNC_X5(0x23B, a, b, c, d)
//            a: u16 Sprite Handle
//            b: u16 X Position
//            c: u16 Y Position
//            d: u8 Duration (fixed duration, variable speed, depends on
//            distance)
...
#define ERAPI_SpriteFindClosestSprite(a, b, c)                                 \
  ERAPI_FUNC_X4(0x2AA, a, b, (u32)c)
//            a: u16 Sprite Handle
//            b: u8 Type
//            c: u16* Distance (0 if no sprites of specified type exist)
//            Return: u16 Sprite Handle (187 if no sprites of specified type
//            exist)
...
//            a: u16 Sprite Handle
//            b: u8 Bank Index
// Pokèmon system sprites have 3 banks, 0 = facing left, 1 = facing front, 2 =
// facing right. typedef struct _ERAPI_SPRITE ERAPI_SPRITE;

#define ERAPI_SpriteSetHitboxSize(a, b, c)                                     \
  ERAPI_FUNC_X4(0x258, a, ((b) >> 1), ((c) >> 1))
//            a: u16 Sprite Handle
//            b: u8 Width
//            c: u8 Height
...
struct _ERAPI_BACKGROUND_DRAW_NUMBER {
  ...
  u8 x;                     // x in tiles
  u8 y;                     // y in tiles
  u16 system_sprite_number; // system sprite to use (4107, 4112, 4120, 4121,
                            // 4135, 4137, 4141, 4142, 4149, 4151, 4157, 4189,
                            // 4191, 4196, 4200, 4201, 4202 or 4203)
  u8 amount_digits;         // amount of digits to draw
  u8 amount_extra_zeroes;   // amount of extra zero digits to draw on the right
                            // (so basically draws number times 10, 100, 1000,
                            // ...)
...
#define ERAPI_BackgroundDrawNumber(num_struct)                                 \
  ERAPI_FUNC_X2(0x26B, (u32)num_struct)

#define ERAPI_BackgroundDrawNumberNewValue(num_struct, new_value)              \
  ERAPI_FUNC_X3(0x26C, (u32)num_struct, new_value)

#define ERAPI_BackgroundDrawNumberDeltaValue(num_struct, delta_value)          \
  ERAPI_FUNC_X3(0x26D, (u32)num_struct, delta_value)
...
struct _ERAPI_BACKGROUND_DRAW_TIME {
  ...
  u8 format; // 0="mm:ss:ff" 1="mm:ss" 2="mmss" 3="mm" 4="mm:ss" 5="mm" 6="mm"
             // 7="" (mm: minutes, ss: seconds: ff: hundreds of a second)
